...
CSkeleton::CSkeleton(): m_pColTile(nullptr), m_pPlayer(dynamic_cast<CPlayer*>(OBJMGR->GetPlayer())), m_pNextPath(nullptr), m_bIsDieSound(false),
m_fCheckDelay(0.f), m_bIsDelayed(false), m_bIsWait(false), m_fWaitTime(0.f),
m_pAstar(ASTARMGR){
}
...
void CSkeleton::Move(){
	...
	if(nullptr != m_pNextPath){
		...
		//이동거리 계산
		D3DXVECTOR3 vSpeed = m_tInfo.vDir * 100.f * m_pTimeMgr->GetDeltaTime();
...
void CSkeleton::SetAmbush(){
	m_ImgFrame = 0;
	m_wstrStateKey = L"Ambush";
}

HRESULT CSkeleton::Release(){
	for(auto& elem : m_PathList)
		SafeDelete(elem);
	m_PathList.clear();
...
HRESULT CSkeleton::Initialize(){
	...
	m_wstrObjectKey = L"SkeletonSword";
	m_wstrStateKey = L"Idle";
	m_wstrDirectionKey = L"Down";

	m_tStat.szName = L"SKELETON";
	m_tStat.szJob = L"";
...
HRESULT CSkeleton::Update(){
	...
	if(!m_bIsDead){
		...
		if(m_wstrStateKey != L"Die"){
			...
			if(400.f > fDistance &&  L"Ambush" != m_wstrStateKey && (m_PathList.empty() || *m_PathList.back() != m_pColTile->GetTile(m_pPlayer->GetInfo().vPos)->vPos)){
				if(nullptr == m_pNextPath)
		...
		if(m_wstrStateKey == L"Walk"){
			...
			if(m_wstrStateKey == L"Die" && !m_bIsDieSound){
				m_pSoundMgr->PlaySound(L"Skeleton.wav", CSoundManager::MONSTER);
				m_bIsDieSound = true;
			}
			CGameObject::Animation();
			if(m_bIsImgEnd){
		...
		} else if(L"Ambush" == m_wstrStateKey){
			CGameObject::Animation();
			if(m_bIsImgEnd){
				m_ImgFrame = 0;
				m_wstrStateKey = L"Idle";
...
CSkeleton* CSkeleton::Create(const D3DXVECTOR3 & _vPos, CColTile* _pColTile){
