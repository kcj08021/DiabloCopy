...
CButcher::CButcher(): m_pColTile(nullptr), m_pPlayer(dynamic_cast<CPlayer*>(OBJMGR->GetPlayer())), m_pNextPath(nullptr),
m_fCheckDelay(0.f), m_bIsDelayed(false), m_bIsWait(false), m_fWaitTime(0.f), m_bIsSoundOut(false), m_bIsAttack(false),
m_pAstar(ASTARMGR){
}
...
//공격
void CButcher::Attack(){
	//현재 공격 모션 종료시 딜레이 시작
	if(m_bIsImgEnd){
		m_wstrStateKey = L"Idle";
		m_bIsDelayed = true;
		m_bIsAttack = false;
	} else if(m_ImgCount - 4 == CGameObject::Animation(0.04f) && !m_bIsAttack){
...
HRESULT CButcher::Initialize(){
	m_tInfo.vSize = { 2.f, 2.f, 0.f };
	m_tInfo.vPos = { 0.f, 0.f, 0.f };
	m_tInfo.vDir = { 0.f, 1.f, 0.f };

	m_wstrTypeKey = L"Boss";
	m_wstrObjectKey = L"Butcher";
	m_wstrStateKey = L"Idle";
	m_wstrDirectionKey = L"Down";

	m_tStat.szName = L"THE BUTCHER";
	m_tStat.szJob = L"";
	...
	if(!m_bIsDead){
		...
		if(m_wstrStateKey != L"Die"){
			...
			if(m_wstrStateKey == L"Idle" && 500.f > fDistance && (m_PathList.empty() || *m_PathList.back() != m_pColTile->GetTile(m_pPlayer->GetInfo().vPos)->vPos)){
				if(!m_bIsSoundOut){
					m_pSoundMgr->PlaySound(L"Butcher.wav", CSoundManager::MONSTER);
					m_bIsSoundOut = true;
				}
...
CButcher* CButcher::Create(const D3DXVECTOR3 & _vPos, CColTile* _pColTile){
	CButcher* pInstance = new CButcher;
