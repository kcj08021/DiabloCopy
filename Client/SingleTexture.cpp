...
CSingleTexture* CSingleTexture::Create(const wstring& _wstrFilePath, const DWORD _TransColor, const D3DXVECTOR3& _vecCenter){
	CSingleTexture* pTextrue = new CSingleTexture;

	if(FAILED(pTextrue->LoadTexture(_wstrFilePath, L"", 0, _TransColor, _vecCenter))){
		SafeDelete(pTextrue);
		return nullptr;
	}
