...
HRESULT CShield::LateInit(){
	m_tStat.iArmor = m_iIndex + 1;
	return S_OK;
}

HRESULT CShield::Update(){
	if(!m_bIsInit){
	...
	if(m_ImgFrame == 0 && !m_bIsSoundOut){
		m_pSoundMgr->PlaySound(L"ShieldDrop.wav", CSoundManager::EFFECT);
		m_bIsSoundOut = true;
	}
...
CShield * CShield::Create(const D3DXVECTOR3 & _vPos /*= D3DXVECTOR3{}*/){
