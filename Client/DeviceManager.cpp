...
void CDeviceManager::Release(){
	...
	if(m_pDibloSmallFont->Release())
		MessageBox(0, L"m_pDibloSmallFont Release Failed", L"ERROR", MB_OK);
	if(m_pDibloBigFont->Release())
		MessageBox(0, L"m_pDibloBigFont Release Failed", L"ERROR", MB_OK);
	if(m_pSprite->Release())
		MessageBox(0, L"m_pSprite Release Failed", L"ERROR", MB_OK);
...
HRESULT CDeviceManager::InitDevice(MODE _eMode){
	...
	tFontInfo.Height = 30;
	tFontInfo.Weight = FW_HEAVY;
	tFontInfo.CharSet = ANSI_CHARSET;
	lstrcpy(tFontInfo.FaceName, L"Exocet");
	hr = D3DXCreateFontIndirect(m_pDevice, &tFontInfo, &m_pDibloSmallFont);
	if(FAILED(hr)){
		::MessageBox(g_hWnd, L"DEVICEMGR->InitDevice() Font Create Failed", L"Error", MB_OK);
		return E_FAIL;
	}
	...
	tFontInfo.CharSet = ANSI_CHARSET;
	lstrcpy(tFontInfo.FaceName, L"Exocet");
	hr = D3DXCreateFontIndirect(m_pDevice, &tFontInfo, &m_pDibloBigFont);
	if(FAILED(hr)){
		::MessageBox(g_hWnd, L"DEVICEMGR->InitDevice() Font Create Failed", L"Error", MB_OK);
		return E_FAIL;
	}
