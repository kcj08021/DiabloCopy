...
void CMPPotion::PotionUsed(){
	m_pSoundMgr->PlaySound(L"InvenPotion.wav", CSoundManager::EFFECT);
	m_pPlayer->GetStat().iMp = m_pPlayer->GetStat().iMaxMp;
}

HRESULT CMPPotion::Release(){
	return S_OK;
}

HRESULT CMPPotion::Initialize(){
	m_tInfo.vSize = { 2.f, 2.f, 0.f };
	m_tInfo.vPos = { 0.f, 0.f, 0.f };
	m_tInfo.vDir = { 0.f, 1.f, 0.f };

	m_wstrTypeKey = L"Item";
	m_wstrObjectKey = L"MPPotion";
	...
	m_tStat.szName = L"POTION OF MANA\nFULLY RECOVER MANA\nRIGHT CLICK TO USE";
...
HRESULT CMPPotion::LateInit(){
	return S_OK;
}

HRESULT CMPPotion::Update(){
	if(!m_bIsInit){
...
HRESULT CMPPotion::LateUpdate(){
	return S_OK;
}

HRESULT CMPPotion::Render(){
...
CMPPotion* CMPPotion::Create(const D3DXVECTOR3 & _vPos, CPlayer* _pPlayer){
	CMPPotion* pInstance = new CMPPotion;
