...
CSkeletonKing::CSkeletonKing(): m_pColTile(nullptr), m_pPlayer(dynamic_cast<CPlayer*>(OBJMGR->GetPlayer())), m_pNextPath(nullptr),
m_fCheckDelay(0.f), m_bIsDelayed(false), m_bIsWait(false), m_fWaitTime(0.f), m_bIsSoundOut(false),
m_fCheckSkillDelay(0.f), m_bIsSkillDelayed(false),
m_pAstar(ASTARMGR){
}
...
//공격
void CSkeletonKing::Attack(){
	//현재 공격 모션 종료시 딜레이 시작
	if(m_bIsImgEnd){
		m_wstrStateKey = L"Idle";
		m_bIsDelayed = true;
		m_bIsAttack = false;
	} else if(m_ImgCount - 7 == CGameObject::Animation(0.04f)&&!m_bIsAttack){
		...
		if(m_wstrDirectionKey == L"Down")
			...
			index += -(TILEX << 1) - 1;
		//타일에 데미지 설정
		m_pColTile->SetDamageTile(index, 20);
		m_bIsAttack = true;
	}
}

void CSkeletonKing::Skill(){
	CGameObject::Animation();
	if(m_bIsImgEnd){
		m_ImgFrame = 0;
		m_wstrStateKey = L"Idle";
		m_bIsSkillDelayed = true;
		size_t index = m_pColTile->GetIndex(m_tInfo.vPos);
		CGameObject* pObj[2] = { nullptr, nullptr };

		if(m_pColTile->GetTile(index + 1)->bIsColi)
			pObj[0] = CSkeleton::Create(m_pColTile->GetTile(index + 1)->vPos, m_pColTile);
		if(m_pColTile->GetTile(index + 80)->bIsColi){
			if(nullptr == pObj[0])
				pObj[0] = CSkeleton::Create(m_pColTile->GetTile(index + 80)->vPos, m_pColTile);
			else
				pObj[1] = CSkeleton::Create(m_pColTile->GetTile(index + 80)->vPos, m_pColTile);
		}
		if(m_pColTile->GetTile(index - 1)->bIsColi){
			if(nullptr == pObj[0])
				pObj[0] = CSkeleton::Create(m_pColTile->GetTile(index - 1)->vPos, m_pColTile);
			else if(nullptr == pObj[1])
				pObj[1] = CSkeleton::Create(m_pColTile->GetTile(index - 1)->vPos, m_pColTile);
		}
		if(m_pColTile->GetTile(index - 80)->bIsColi){
			if(nullptr == pObj[0])
				pObj[0] = CSkeleton::Create(m_pColTile->GetTile(index - 1)->vPos, m_pColTile);
		...
		for(auto elem : pObj)
			if(nullptr != elem){
				dynamic_cast<CSkeleton*>(elem)->SetAmbush();
				OBJMGR->AddObject(CObjectManager::OBJ_MONSTER, elem);
			}
	}
}

HRESULT CSkeletonKing::Release(){
	for(auto& elem : m_PathList)
		SafeDelete(elem);
	m_PathList.clear();
...
HRESULT CSkeletonKing::Initialize(){
	m_tInfo.vSize = { 2.f, 2.f, 0.f };
	m_tInfo.vPos = { 0.f, 0.f, 0.f };
	m_tInfo.vDir = { 0.f, 1.f, 0.f };

	m_wstrTypeKey = L"Boss";
	m_wstrObjectKey = L"SkeletonKing";
	m_wstrStateKey = L"Idle";
	m_wstrDirectionKey = L"Down";

	m_tStat.szName = L"SKELETON KING";
	m_tStat.szJob = L"";
...
HRESULT CSkeletonKing::LateInit(){
	m_pColTile->SetColl(m_pColTile->GetIndex(m_tInfo.vPos), false);
	m_pColTile->SetPreemTile(m_tInfo.vPos, this);
...
HRESULT CSkeletonKing::Update(){
	...
	if(!m_bIsDead){
		...
		if(m_wstrStateKey != L"Die"){
			...
			if(m_wstrStateKey == L"Idle" && 500.f > fDistance && (m_PathList.empty() || *m_PathList.back() != m_pColTile->GetTile(m_pPlayer->GetInfo().vPos)->vPos)){
				if(!m_bIsSoundOut){
					m_pSoundMgr->PlaySound(L"SkeletonKing.wav", CSoundManager::MONSTER);
					m_bIsSoundOut = true;
				}
				
				if(nullptr == m_pNextPath)
					m_pAstar->PathFinding(m_PathList, m_tInfo.vPos, m_pPlayer->GetInfo().vPos);
				else
					m_pAstar->PathFinding(m_PathList, *m_pNextPath, m_pPlayer->GetInfo().vPos);
			...
			} else if(500.f > fDistance){
				if(!m_bIsSkillDelayed){
					if(m_wstrStateKey != L"Special")
						m_ImgFrame = 0;
					m_wstrStateKey = L"Special";
				} else{
					m_fCheckSkillDelay += m_pTimeMgr->GetDeltaTime();
					if(m_fCheckSkillDelay >= 3.f){
						m_bIsSkillDelayed = false;
						m_fCheckSkillDelay = 0;
					}
				}
			}
		}
		...
		} else if(m_wstrStateKey == L"Special"){
			if(!m_bIsSkillDelayed)
				Skill();
		} else{
			CGameObject::Animation();
...
HRESULT CSkeletonKing::LateUpdate(){
	if(m_wstrStateKey != L"Die"){
		int iDamage = 0;
...
CSkeletonKing* CSkeletonKing::Create(const D3DXVECTOR3 & _vPos, CColTile* _pColTile){
	CSkeletonKing* pInstance = new CSkeletonKing;
