...
CFallenOne::CFallenOne(): m_pColTile(nullptr), m_pPlayer(dynamic_cast<CPlayer*>(OBJMGR->GetPlayer())), m_pNextPath(nullptr), m_bIsDieSound(false),
m_fCheckDelay(0.f), m_bIsDelayed(false), m_bIsWait(false), m_fWaitTime(0.f),
m_pAstar(ASTARMGR){
}


CFallenOne::~CFallenOne(){
	Release();
}
//이동
void CFallenOne::Move(){
	//현재 이동 목표가 없을때
	if(nullptr != m_pNextPath){
		...
		//이동거리 계산
		D3DXVECTOR3 vSpeed = m_tInfo.vDir * 150.f * m_pTimeMgr->GetDeltaTime();
...
void CFallenOne::Attack(){
	...
	if(m_bIsImgEnd){
		...
		if(m_wstrDirectionKey == L"Down")
			...
			index += -(TILEX << 1) - 1;
		//타일에 데미지 설정
		m_pColTile->SetDamageTile(index, 8);
		m_bIsAttack = true;
	}
}

HRESULT CFallenOne::Release(){
	for(auto& elem : m_PathList)
		SafeDelete(elem);
	m_PathList.clear();
...
HRESULT CFallenOne::Initialize(){
	m_tInfo.vSize = { 2.f, 2.f, 0.f };
	m_tInfo.vPos = { 0.f, 0.f, 0.f };
	m_tInfo.vDir = { 0.f, 1.f, 0.f };

	m_wstrTypeKey = L"Monster";
	m_wstrObjectKey = L"FallenOneSword";
	m_wstrStateKey = L"Idle";
	m_wstrDirectionKey = L"Down";

	m_tStat.szName = L"FALLEN ONE";
	m_tStat.szJob = L"";
...
CFallenOne* CFallenOne::Create(const D3DXVECTOR3 & _vPos, CColTile* _pColTile){
	CFallenOne* pInstance = new CFallenOne;
